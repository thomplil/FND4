function Step2_VolumeSourceModeling_bandLCMVbeamform_EGI(subjects,input_type)

%Author: Ravi Mill, rdm146@rutgers.edu, adapted by Lily Thompson
%let83@rutgers.edu
%Last update: Aug 5th 2020

%DESCRIPTION: source models EEG data (task or rest) for CPRO2_EEG
...
addpath /projects/f_mc1689_1/cpro2_eeg/docs/toolboxes/fieldtrip-20240704/external/spm8/;
...
input_dir='/projects/f_mc1689_1/cpro2_eeg/data/results/preproc1_causalFilter/';

%path to egi electrode layout (in Fieldtrip templates directory)
elecfile='/projects/f_mc1689_1/cpro2_eeg/docs/toolboxes/fieldtrip-20240704/template/electrode/GSN-HydroCel-257.sfp';

%Load power atlas coords - sorted into functional networks (to help
%visualisation later) -- CAN I JUST COPY OVER THIS FILE ?
...
%*Set input suffix - refer to preproc for parms to enter here -- NOT SURE
...
MRI_dir='/projects/f_mc1689_1/CPRO2_learning/data/rawdata/';
MRI_input='/MRI/T1.nii.gz'; %NIFTI not normalised, with skull -- CAN'T FIND WHERE THE NIFTI FILE IS STORED IN CPRO2_LEARNING

%now setting MNI_template_sourcemodel to regular 4mmgrid MNI created by Fieldtrip users
ftrip_MNI_template='/projects/f_mc1689_1/cpro2_eeg/docs/toolboxes/fieldtrip-20240704/template/sourcemodel/standard_sourcemodel3d4mm.mat'; %wholebrain grid (used when modeling wholebrain grid and then parcellating)
...
%*set frequency bands to beamform in -- DO WE WANT TO CHANGE THIS ?
...
%hilbert_apply=0; -- SHOULD WE USE THE HILBERT TRANSFORM?
...
%create head model output directories - path has to point to previously
%created head models -- I DON'T HAVE THESE ?
MRI_suffix = 'MRIheadmodel';
fieldtrip_processMRI='/projects/f_mc1689_1/cpro2_eeg/data/results/Analysis2_DynamicDecoding_reproc/Stim_locked/Fieldtrip_headmodels_MRI/';
...
for i = 1:length(subjects)
    ...
    if input_type==1
        data_final = data_task_seg_encode; %FIX
        clear data_task_seg_encode;
        input_name='task'; %used to write output
    elseif input_type==2
        data_final=rest_data_seg_channels_trials_autovisIC;
        input_name='rest';
        clear rest_data_seg_channels_trials_autovisIC;
    end
   ...
   %% Set dipole location based on MNI transformations
    ...
    if strcmp(source_grid,'Whole')
        ...
        if ~exist(subject_grid)
            cfg = [];
            cfg.warpmni = 'yes';
            cfg.template = template_grid; %Note that even though template_grid is in mm, this step yields a grid in cm (that is the same as if we had converted units of template_grid to cm before)
            cfg.nonlinear = 'no'; %*works way better with nonlinear transform set to no!!
            cfg.mri = aligned_mri;
    ...
    elseif strcmp(source_grid,'Power') %THIS IS WHERE IT CHANGES FOR SURFACE
        ...
        if ~exist(subject_grid)
            ...
            cfg.warpmni='yes';
            cfg.template=template_grid; %Note that even though template_grid is in mm, this step yields a grid in cm (that is the same as if we had converted units of template_grid to cm before)
            cfg.nonlinear='no'; %*works better with nonlinear transform set to no!!
            cfg.mri=aligned_mri;
            cfg.inwardshift=inwardshift_parm;
            grid=ft_prepare_sourcemodel(cfg);
        ...
        for f=1:num_bands
            ...
            for demean=1:length(demean_detrend)
                ...
                %% 2. Run through beamformer covariance computation - Task + Rest common filter
                    ...
                    cfg.headmodel = vol; %CHANGE TO SOURCEMODEL FOR SURFACE
                    ...
                    source_filt=[output_dir,num2str(subjects(i,1)),'_',input_name,'_sourceFilter_withlf_volume',output_suffix,'_',num2str(current_freq(1,1)),'to',num2str(current_freq(1,2)),'Hz.mat'];
                    if ~exist(source_filt)
                        source_data_volume_covariance = ft_sourceanalysis(cfg,data_covariance);

                        %save
                        save(source_filt,'source_data_volume_covariance','-v7.3');
                    else
                    ...
                    cfg.keepfilter = 'no';
                    sourcetimeseries_volume_task_commonfilter = ft_sourceanalysis(cfg,task_freqband_avg);   
                    ...
                    if current_freq(1,1)==0.5 %treatment for '0.5' being written for delta output
                        save([output_dir,num2str(subjects(i,1)),'_',input_name,'_',output_source_trial_prefix,output_suffix,'_0pt5to',num2str(current_freq(1,2)),'Hz.mat'],'sourcetimeseries_volume_task_commonfilter','beam_filter','-v7.3');

                    else
                        save([output_dir,num2str(subjects(i,1)),'_',input_name,'_',output_source_trial_prefix,output_suffix,'_',num2str(current_freq(1,1)),'to',num2str(current_freq(1,2)),'Hz.mat'],'sourcetimeseries_volume_task_commonfilter','beam_filter','-v7.3');
                    end
