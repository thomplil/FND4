function Step1_preproc_EEG_rest_task_RM_v2(subjects, run_ica_inspect)
...
for x = 1:length(subjects)
    ...
    %name output files 
    %~~ we don't need timingfile for running the code finally, so I would
    %comment out where the variable is saved; note that on Amarel
    %we not only have a limit on storage space, but also on the number of
    %files, so that's one motivation for not saving timingfile...
    ...
    %~~the . introduced by filter_hp could cause problems loading in the
    %data later; so you can replace it using the regexp command below
    %~~ also - pedantic point but I don't think there's a need for the '_'
    %after 'sub', so I've removed that now
    outputfile = [outputDir,'sub',num2str(subjects(1,x)),'_preproc_hp',num2str(filter_hp),'notch_seg_autochannelz',num2str(meanabs_channel_thresh),'_trialz',num2str(meanabs_trial_thresh),'_ICAz',num2str(ica_tempcorr_thresh),'_baselinelp',num2str(filter_lp),'avgref'];
    outputfile = regexprep(outputfile,'\.','pt');
    %~~ add the .mat (would've caused confusion if added prior to regexp)
    ...
    ICA_output = [outputDir,'sub',num2str(subjects(1,x)),'_segICAcomps_ica_extended_channeltrialthresh',num2str(meanabs_channel_thresh),'.mat'];
...
%             tt_out=tt_event;
   ...
   %% 4. Perform ICA 
    ...
    if exist(ICA_output)
        ...
        %sample script I sent you) -- STILL DECIDING
    ...
    %FOR TESTING PCA.REDUCE VS NO PCA.REDUCE
    % tic
    % disp('starting ICA -- runica extended');
    % cfg = [];
    % cfg.method = 'runica';
    ...
    %  [comp] = ft_componentanalysis(cfg,data_task_seg_1);
    %  disp('got through ft_component analysis, attempting save output')
    %  save(ICA_output_NO_PCA, 'comp','-v7.3');
    %  disp('saved output')
    %  toc
    %  fprintf('Running runica .pca = 150 took %.2f seconds\n', toc);
