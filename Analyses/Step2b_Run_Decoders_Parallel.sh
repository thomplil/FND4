...
subjects="1 10 16"  # Subject lists
# subjects="5 2 3"
# subjects="17 21 34 44" 
decoderType_list=("SVM" "LDA") # "Random_Forest" "SVM" "LDA"
#For running from FND4
...
#For running from cpro2_eeg
scriptDir="/projects/f_mc1689_1/cpro2_eeg/docs/scripts/TempGenPython" # or subjBatch_runSingleDecoder
fxn_dir="/projects/f_mc1689_1/cpro2_eeg/docs/scripts/"  # Directory containing functions
...
# Loop over each subject and decoder type
for decoderType in "${decoderType_list[@]}"; do
    for subjNum in $subjects; do
        # Generate the Python script for each subject
        python_script_path="${scriptDir}/${subjNum}_${decoderType}_decoder_script.py"
...
from Step2a_run_decoders_fxns_v2 import DecodingAcc, decode_single_timeseries, decode_TempGen

# Decoding Analysis Function
decodingAnalysis = 'TempGen'  # or 'DecodingAccuracy' (done)
classify_cond = 'Left' # or 'Right' (needs "17 21 34 44", 5svm) or 'Hand' (lda needs last 4, svm has 1 10 16 (2 3 5 17)); left lda and svm needs "17 21 34 44", rf needs rest
subjNum = ${subjNum}
...
EOL
        ...
        slurm_script_path="${scriptDir}/${subjNum}_${decoderType}_slurm_script.sh"
        echo "Creating SLURM job script for subject $subjNum at $slurm_script_path"
        # Make memory allocation conditional 
        if [ "$decoderType" = "LDA" ]; then
            #changed for tempGen
            numCPUs=15
            memory=20000
        elif [ "$decoderType" = "SVM" ]; then
            #changed for tempGen
            numCPUs=27
            memory=15000
        elif [ "$decoderType" = "Random_Forest" ]; then
            #changed for tempGen
            numCPUs=26
            memory=40000
        fi
...
#SBATCH --job-name=${decoderType}_${subjNum}
#SBATCH --time=10:00:00      # at increased for tempGen
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=${numCPUs}     # Number of parallel tasks, adjust as needed for tempGen
#SBATCH --mem=${memory}
#SBATCH --output=${scriptDir}/${subjNum}_${decoderType}.out
#SBATCH --error=${scriptDir}/${subjNum}_${decoderType}.err
#SBATCH --export=ALL
