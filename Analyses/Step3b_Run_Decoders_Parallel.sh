...
from Step2a_run_decoders_fxns import decode_single_timeseries, decode_TempGen
...
if decodingAnalysis == 'DecodingAccuracy':
    ...
    avg_accuracy, sem_accuracy = Parallel(n_jobs=-1)(delayed(decode_single_timeseries)(classify_cond, subjNum,t, decoderType) for t in range(nTimepoints))
    # Import functions from Step2a_run_decoders_fxns
    import Step2a_run_decoders_fxns
    from Step2a_run_decoders_fxns import decode_single_timeseries, decode_TempGen
    ...
    tempGenAcc = Parallel(n_jobs=-1)(delayed(decode_TempGen)(t, nTimepoints, classify_cond, subjNum, decoderType) for t in range(nTimepoints))
