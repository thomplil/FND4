function Step2b_Raw_MVPA_TempGen_subject(total_subjects)    
%Step4_DynamicDecoding_RawSensor_subject_pseudotrial_v2.m
    ...
    %total_subjects = [1]; %total_subjects and subjects specification helps load in behav files
    ...
    % identify trials using the key chosen, respMotorHand, RespIndexMiddle,
    ...
    %     end
       ...
       %% Response lock the data
        ...
        tic;
        time_tempGen = repmat(struct('fold', [], 'ptrial', [], 'time', []), ...
                      run_pseudotrials, 10); % or infer max_nfolds dynamically
        ...
        for xx=1:run_pseudotrials
            ...
            for nn=1:length(partitions.test_indices)
                ...
                time_tempGen(xx,nn).fold = nn;
                time_tempGen(xx,nn).ptrial = xx;
                time_tempGen(xx,nn).time = toc;
            end
        ...
        save([output_dir,num2str(subjects(i)),'timingInfoOfClassification.mat'], 'time_tempGen');
